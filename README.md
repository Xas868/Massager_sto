...
- **imageLink** - ссылка на фото пользователя
- **nickname** - никнейм(прозвище) пользователя
- **isDeleted** - флаг удалён ли пользователь
- **
```
...
- **commentQuestions** - список комментариев на вопрос
- **userFavoriteQuestions** - список любимых вопросов пользователя
- **voteQuestions** - список голосов вопроса
```
...
#### Поля
- **id** - уникальный идентификационный номер голоса вопроса;
...
- **countVQ** - количество голосов вопроса. Не хранится в базе помечен аннотацией @Transient
```
...
- **updateDateTime** - дата последнего изменения ответа;
...
- **editModerator** - пользователь который может изменят ответ;
- **htmlBody** - тело ответа;
- **isHelpful** - отметка, что именно этот ответ помог решить вопрос, к которому оно относиться. Эту отметку может ставить только автор вопроса;
...
- **commentAnswers** - список комментариев на ответ;
- **voteAnswers** - список голосов ответа
```
...
#### Поля
- **id** - уникальный идентификационный номер голоса ответа;
- **user** - пользователь, который отправил свой голос;
...
- **title** - заглавие
- **bookMarks** - множество(уникальный список) закладок
```
Группа закладок
```

[Схема](https://dbdiagram.io/d/63a0b66499cb1f3b55a25557)
