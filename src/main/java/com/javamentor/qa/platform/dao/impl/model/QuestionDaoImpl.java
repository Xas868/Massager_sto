...
public class QuestionDaoImpl extends ReadWriteDaoImpl<Question, Long> implements QuestionDao {
    ...
    public Optional<List<Long>> getAllQuestionIdByTagId(Long id) {
        return Optional.ofNullable(
                entityManager.createNativeQuery("select q.question_id from question_has_tag q where q.tag_id = :id")
                .setParameter("id", id)
                .getResultList()
        );
    }
    ...
    public Long getCountOfViewByQuestionId(Long id) {
        return entityManager.createQuery("select count (qv.id) from QuestionViewed qv where qv.question.id = id", Long.class)
                .setParameter("id", id)
                .getSingleResult();
    }
    ...
    public Long getCountOfVoteByQuestionId(Long id) {
        return entityManager.createQuery("select sum(case when v.vote = 'UP_VOTE' then 1 else -1 end) from VoteQuestion v where v.question.id = id", Long.class)
                .setParameter("id", id)
                .getSingleResult();
    }
}