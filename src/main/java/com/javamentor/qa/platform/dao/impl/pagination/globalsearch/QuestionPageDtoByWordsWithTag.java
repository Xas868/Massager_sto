...
@Repository("QuestionPageDtoByWordsWithTag")
public class QuestionPageDtoByWordsWithTag implements PageDtoDao<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getPaginationItems(PaginationData properties) {
        String q =(String) properties.getProps().get("q");
        String[] qArray = q.split("](?!.*])");
        List<String> tags = Arrays.asList(qArray[0].replaceFirst("\\[","").split("][\\+| ]*\\["));
        ...
        return entityManager.createQuery(
                        "select " +
                                ...
                                " WHERE lower(q.title) like lower(concat('%',:q,'%')) OR" +
                                " lower(q.description) like lower(concat('%',:q,'%')) AND" +
                                " ((:tags) IS NULL OR q.id IN (select q.id from Question q join q.tags t where" +
                                " t.name in (:tags) GROUP BY q.id HAVING COUNT(*) = (:tagsSize)))" +
                                " ORDER BY q.persistDateTime desc"
                )
                .setParameter("q", qArray[1])
    ...
    public Long getTotalResultCount(Map<String, Object> properties) {
        String q =(String) properties.get("q");
        String[] qArray = q.split("](?!.*])");
        List<String> tags = Arrays.asList(qArray[0].replaceFirst("\\[","").split("][\\+| ]*\\["));

        return (Long) entityManager.createQuery(
                 "select distinct count(distinct q.id) from Question q" +
                        " WHERE lower(q.title) like lower(concat('%',:q,'%')) OR" +
                        " lower(q.description) like lower(concat('%',:q,'%')) AND" +
                        " ((:tags) IS NULL OR q.id IN (select q.id from Question q join q.tags t where" +
                        " t.name in (:tags) GROUP BY q.id HAVING COUNT(*) = (:tagsSize)))"
