...
@Repository("QuestionPageDtoDaoByAnswer")
public class QuestionPageDtoDaoByAnswer implements PageDtoDao<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getPaginationItems(PaginationData properties) {
        String q = ((String) properties.getProps().get("q")).replaceAll("[^\\d]","");
        ...
        return entityManager.createQuery(
                        "select " +
                                ...
                                " WHERE (select count (a.id) from Answer a where a.question.id = q.id) <= :q" +
                                " ORDER BY q.persistDateTime desc"
                )
                .setParameter("q",Long.parseLong(q))
    ...
    public Long getTotalResultCount(Map<String, Object> properties) {
        ...
        String q = ((String) properties.get("q")).replaceAll("[^\\d]","");
