...
@Repository("QuestionPageDtoDaoByExactPhrase")
public class QuestionPageDtoDaoByExactPhrase implements PageDtoDao<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getPaginationItems(PaginationData properties) {
        ...
        return entityManager.createQuery(
                        "select " +
                                ...
                                " WHERE lower(q.title) like lower(concat('%',:q,'%')) OR lower(q.description) like lower(concat('%',:q,'%'))" +
                                " ORDER BY q.persistDateTime desc"
                )
                .setParameter("q",((String) properties.getProps().get("q")).replaceAll("\"",""))
    ...
    public Long getTotalResultCount(Map<String, Object> properties) {
        ...
        return (Long) entityManager.createQuery("select distinct count(distinct q.id) from Question q WHERE " +
                        "lower(q.title) like lower(concat('%',:q,'%')) OR lower(q.description) like lower(concat('%',:q,'%'))"
                )
                .setParameter("q", ((String)properties.get("q")).replaceAll("\"",""))
                .getSingleResult();
    }
}
