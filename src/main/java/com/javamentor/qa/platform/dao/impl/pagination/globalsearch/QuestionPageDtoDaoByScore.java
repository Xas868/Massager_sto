...
@Repository("QuestionPageDtoDaoByScore")
public class QuestionPageDtoDaoByScore implements PageDtoDao<QuestionViewDto> {
    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public List<QuestionViewDto> getPaginationItems(PaginationData properties) {
        String q = ((String) properties.getProps().get("q")).replaceAll("score:","");
        String jpql;
        if(q.endsWith("..")){
            jpql = ">="+q.replaceAll("[^\\d]","");
        } else {
        ...
        return entityManager.createQuery(
                        "select " +
                                ...
                                " WHERE (select sum(case when v.vote = 'DOWN_VOTE' then -1 when v.vote = 'UP_VOTE' then 1 else 0 end)" +
                                " from VoteQuestion v where v.question.id=q.id) " + jpql +
    ...
    public Long getTotalResultCount(Map<String, Object> properties) {
        ...
        String q = ((String) properties.get("q")).replaceAll("score:","");
        String jpql;
        if(q.endsWith("..")){
            jpql = ">="+q.replaceAll("[^\\d]","");
        } else {
