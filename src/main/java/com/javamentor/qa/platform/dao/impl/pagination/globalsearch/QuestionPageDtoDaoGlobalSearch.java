...
@Repository("QuestionPageDtoDaoGlobalSearch")
public class QuestionPageDtoDaoGlobalSearch implements PageDtoDao<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getPaginationItems(PaginationData properties) {
        ...
        return entityManager.createQuery(
                ...
                .setParameter("q", properties.getProps().get("q"))
    ...
    public Long getTotalResultCount(Map<String, Object> properties) {
        ...
        return (Long) entityManager.createQuery("select distinct count(distinct q.id) from Question q join q.tags t WHERE " +
                "lower(q.title) like lower(concat('%',:q,'%')) OR lower(q.description) like lower(concat('%',:q,'%')) OR" +
                " ((:q) is null or q.id in (select q.id from Question q join q.tags t where lower(t.name) like lower(concat('%',:q,'%'))))"
        )
                .setParameter("q", properties.get("q"))
                .getSingleResult();
    }
}
